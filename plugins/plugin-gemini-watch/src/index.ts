...
function parseGeminiSessionFile(
  ...
  const lastToolCall =
    [...messages]
      .reverse()
      .find(message => Array.isArray(message.toolCalls) && message.toolCalls.length > 0) ?? null;
  ...
  const eventType = getString(lastMessage.type) || 'message';

  const summary = getString(lastMessage.content) || getString(record.summary) || 'Gemini activity';

  const detail = getString(record.projectHash) || getString(record.cwd);
