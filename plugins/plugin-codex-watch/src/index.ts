...
function parseRecord(
  ...
  const timestamp =
    getString(record.timestamp) || getString(record.created_at) || fallbackTimestamp;

  const eventType =
    getString(record.event_type) || getString(record.type) || getString(payload?.type) || 'message';
  ...
  const detail =
    getString(payload?.arguments) || getString(record.detail) || getString(record.content);
  ...
  const event = {
    ...
    meta: {
      filePath,
      toolName:
        getString(payload?.name) || getString(record.toolName) || getString(record.tool_name),
