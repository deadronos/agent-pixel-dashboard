...
function parseRecord(sourceHost: string, filePath: string, record: Record<string, unknown>, sequence: number): NormalizedEvent {
  const sessionId =
...
export class CodexWatchPlugin implements CollectorPlugin {
  ...
  async watch(root: DiscoveredSessionRoot, ctx: WatchContext): Promise<WatchHandle> {
    ...
    const ingestFile = async (filePath: string): Promise<void> => {
