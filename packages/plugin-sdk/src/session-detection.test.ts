...
describe('matchesSessionFile', () => {
  it('matches codex rollout files only', () => {
    expect(
      matchesSessionFile('codex', '/Users/test/.codex/sessions/2026/04/09/rollout-abc.jsonl')
    ).toBe(true);
  ...
  it('matches copilot events files', () => {
    expect(
      matchesSessionFile('copilot', '/Users/test/.copilot/session-state/uuid/events.jsonl')
    ).toBe(true);
    expect(
      matchesSessionFile('copilot', '/Users/test/.copilot/session-state/uuid/other.jsonl')
    ).toBe(false);
  });

  it('matches openclaw agent session files', () => {
    expect(
      matchesSessionFile('openclaw', '/Users/test/.openclaw/agents/researcher/sessions/abc.jsonl')
    ).toBe(true);
  ...
  it('matches claude project and subagent jsonl files', () => {
    expect(matchesSessionFile('claude', '/Users/test/.claude/projects/foo/session.jsonl')).toBe(
      true
    );
