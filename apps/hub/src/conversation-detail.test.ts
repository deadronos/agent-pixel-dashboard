...
describe('conversation detail lookup', () => {
  ...
  it('looks up by source + sessionId when provided', () => {
    const sessionEntity = applyEvent(
      undefined,
      sampleEvent({ entityId: 'codex:session:abc123', eventId: 'evt_s1' })
    );
    ...
    const recentEvents: NormalizedEvent[] = [
      sampleEvent({
        ...
        eventId: 'evt_s0',
        timestamp: '2026-04-09T20:15:30.000Z',
      }),
      sampleEvent({
        entityId: toolEntity.entityId,
        eventId: 'evt_t0',
        timestamp: '2026-04-09T20:15:35.000Z',
        entityKind: 'tool-run',
        displayName: 'ls',
      }),
      sampleEvent({
        entityId: 'codex:session:other',
        sessionId: 'other',
        eventId: 'evt_other',
        timestamp: '2026-04-09T20:15:36.000Z',
      }),
    ];
...
describe('conversation detail handler', () => {
  ...
  it('returns a clamped timeline and detail body for a successful lookup', async () => {
    ...
    expect((res.body as any).recentEvents.map((event: NormalizedEvent) => event.eventId)).toEqual([
      'evt_new',
    ]);
