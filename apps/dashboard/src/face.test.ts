...
describe('getStatusFromTimestamp', () => {
  ...
  it('marks older timestamps as dormant', () => {
    expect(getStatusFromTimestamp(new Date(Date.now() - 10 * 60_000).toISOString())).toBe(
      'dormant'
    );
  });
});
...
describe('resolveLiveStatus', () => {
  it('preserves terminal statuses during refresh', () => {
    expect(resolveLiveStatus('done', new Date(Date.now() - 10 * 60_000).toISOString())).toBe(
      'done'
    );
    expect(resolveLiveStatus('error', new Date(Date.now() - 10 * 60_000).toISOString())).toBe(
      'error'
    );
  });
